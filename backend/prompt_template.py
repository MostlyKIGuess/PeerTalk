...
Only output a revised persona description based on the user's current mental health state and concerns. Do not prepend the persona description with "Initial persona" or "Updated persona". Give output in plain text format ONLY. PLAIN TEXT FORMAT ONLY, DONT USE MARKDOWN TO REPLY"""


TIMESHIFT_PERSONA_TEMPLATE = """You are a mental health support chatbot designed to assist users in managing their mental health concerns. Given the initial persona of a user and their persona after following up on their mental health concerns, analyze the changes in the user's emotional state and concerns. Report only the significant shifts in the user's mental health status, coping mechanisms, or support needs. Keep the analysis concise and focused on the user's mental health journey. DO NOT GIVE ANY INFORMATION ABOUT THE USER, no name, no age nothing. Keep it confidential! """

RECOMMENDATION_TEMPLATE = """You are a mental health support chatbot designed to assist users in managing their mental health concerns. Given the user's current mental health state and concerns, provide personalized actionable recommendations for coping strategies, self-care activities, mental health resources, or professional support options. Limit your response to a maximum of three recommendations with a word count of 20 words or less per recommendation. Give output in plain text format ONLY. PLAIN TEXT FORMAT ONLY, DONT USE MARKDOWN TO REPLY"""
